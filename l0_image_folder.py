...
from l0_utils import BAND_LIST, BAND_SPATIAL_RESOLUTION_DICT, DN_MAX, image_band_reshape, IMAGE_SHAPE_DICT
import torch
...
class L0ImageFolder(Dataset):
    ...
    def __init__(self, root, seed, test_train_split, l0_format, target_resolution_merged_m = 20.0, transform=None, preloaded=True, split="train"):
        ...
        train_split_idx=int(test_train_split * len(l0_files))
    ...
    def _get_merged_file_(self, file_path):
        ...
        img=torch.zeros(13, IMAGE_SHAPE_DICT[self.target_resolution_merged_m][0], IMAGE_SHAPE_DICT[self.target_resolution_merged_m][1])

        for n in range(0,12):
            #Reshaping all the bands to the target resolution
            img[n]=image_band_reshape(self._open_band_(os.path.join(file_path, BAND_LIST[n]+".tif")).squeeze(0), BAND_LIST[n], self.target_resolution_merged_m)
    ...
    def _open_band_(self, band_path):
        ...
        with rasterio.open(band_path) as src:
            return torch.from_numpy(src.read(1).astype(np.float32)).unsqueeze(0) / DN_MAX
